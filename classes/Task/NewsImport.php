 ...
 */
class Task_NewsImport extends Minion_Task
{
...
//        set_time_limit(600);
        ...
        $result = DB::select()->from('news_block_section')->order_by('id', 'ASC')->as_assoc()->execute();
        foreach($result as $res)
            $source_categories[$res['name']] = $res['id'];
        ...
        for($i=0; $i*self::ONE_STEP_AMOUNT < self::ALL_AMOUNT; $i++){
            ...
            foreach($result as $row){
                try{
                    /* Category & Part */
                    if(!isset( $cat2cat[$row['id_category']] ))
                        continue;
